...
-- Owner-managed outbound notification credentials. Passwords and API secrets
-- are encrypted by the application before they reach MySQL. The encryption
-- key is supplied separately through VHOME_SECRET_ENCRYPTION_KEY.
CREATE TABLE IF NOT EXISTS household_notification_settings (
    ...
    CONSTRAINT fk_notification_settings_household
        FOREIGN KEY (household_id)
        REFERENCES households (id)
        ON UPDATE RESTRICT
        ON DELETE CASCADE,
    CONSTRAINT fk_notification_settings_updated_by
        FOREIGN KEY (updated_by)
        REFERENCES members (id)
        ON UPDATE RESTRICT
        ON DELETE SET NULL,
    CONSTRAINT chk_notification_settings_smtp_port
        CHECK (smtp_port BETWEEN 1 AND 65535),
    CONSTRAINT chk_notification_settings_smtp_security
        CHECK (smtp_security IN ('TLS', 'STARTTLS')),
    CONSTRAINT chk_notification_settings_sms_provider
        CHECK (sms_provider = 'TENCENT_CLOUD'),
    CONSTRAINT chk_notification_settings_version
        CHECK (version > 0)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_unicode_ci;

-- 家庭备忘事项。recipient_ids 使用 JSON 数组保存需要接收提醒的成员 ID：
-- 当前是单家庭、小数据量场景，这样可以避免仅为提醒状态再拆一张关联表。
-- 成员屏蔽提醒时从数组中移除自己的 ID；数组为空的记录由清理任务删除。
CREATE TABLE IF NOT EXISTS memos (
    ...
    PRIMARY KEY (id),
    KEY idx_memos_household_remind_at (
        household_id,
        remind_at,
        id
    ),
    KEY idx_memos_created_by_remind_at (
        created_by,
        remind_at,
        id
    ),
    KEY idx_memos_pending_email (
        email_sent_at,
        remind_at,
        id
    ),
    KEY idx_memos_pending_sms (
        sms_sent_at,
        remind_at,
        id
    ),

    CONSTRAINT fk_memos_household
        FOREIGN KEY (household_id)
        REFERENCES households (id)
        ON UPDATE RESTRICT
        ON DELETE RESTRICT,

    CONSTRAINT fk_memos_created_by
        FOREIGN KEY (created_by)
        REFERENCES members (id)
        ON UPDATE RESTRICT
        ON DELETE RESTRICT,

    CONSTRAINT chk_memos_title
        CHECK (CHAR_LENGTH(TRIM(title)) > 0),
    CONSTRAINT chk_memos_recipient_ids
        CHECK (JSON_TYPE(recipient_ids) = 'ARRAY'),
    CONSTRAINT chk_memos_version
        CHECK (version > 0)
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8mb4
  COLLATE = utf8mb4_unicode_ci;

-- 普通工作日和周末可由日期直接推导，这张表只记录中国法定节假日及调休工作日。
-- 数据可以按年度从权威来源同步，并用 calendar_date 主键安全地重复写入更新。
