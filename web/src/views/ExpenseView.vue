  ...
  <div class="expense-page page-stack">
    ...
    <section class="ledger-toolbar">
      ...
      <div class="month-switcher">
        <button type="button" aria-label="查看上个月账单" @click="moveMonth(-1)"><ChevronLeft :size="18" /></button>
        <strong>{{ displayedMonth }}</strong>
        <button type="button" aria-label="查看下个月账单" :disabled="isCurrentMonth" @click="moveMonth(1)"><ChevronRight :size="18" /></button>
      </div>
      ...
      <div v-else-if="ledger.records.length" class="expense-record-list">
        <article v-for="record in ledger.records" :key="record.id" class="expense-record-row">
          ...
          <div v-if="canManage(record)" class="record-actions">
            <button type="button" aria-label="编辑这笔支出" data-tooltip-placement="top" @click="openEdit(record)"><Edit3 :size="16" /></button>
            <button class="danger" type="button" aria-label="删除这笔支出" data-tooltip-placement="top-left" @click="remove(record)"><Trash2 :size="16" /></button>
          </div>
        </article>
      </div>
    ...
    <div v-if="modalOpen" class="expense-modal-backdrop" @click.self="modalOpen = false">
      <section class="expense-modal pixel-panel" role="dialog" aria-modal="true" :aria-label="editing ? '编辑支出' : '新增支出'">
        <header><div><p class="panel-kicker">{{ editing ? "EDIT EXPENSE" : "NEW EXPENSE" }}</p><h2>{{ editing ? "修改支出" : "记一笔支出" }}</h2></div><button type="button" aria-label="关闭支出窗口" data-tooltip-placement="left" @click="modalOpen = false"><X :size="20" /></button></header>
        <p v-if="error" class="form-message error">{{ error }}</p>
        <form @submit.prevent="save">
          ...
          <fieldset class="scope-picker">
            ...
            <label class="scope-option">
              <input v-model="form.expense_scope" type="radio" value="PERSONAL" :disabled="!!editing && editing.member_id !== session.member?.id" />
              <span class="scope-option-icon" aria-hidden="true">👤</span>
              <span class="scope-option-copy"><strong>个人开销</strong><small>只在自己的账本中展示</small></span>
              <span class="scope-option-check" aria-hidden="true">✓</span>
            </label>
            <label class="scope-option">
              <input v-model="form.expense_scope" type="radio" value="COLLECTIVE" />
              <span class="scope-option-icon" aria-hidden="true">🏡</span>
              <span class="scope-option-copy"><strong>集体开销</strong><small>家庭成员都能查看</small></span>
              <span class="scope-option-check" aria-hidden="true">✓</span>
            </label>
          </fieldset>
